...
//defining some stucts and enums used below
...
//Functions for Accessing Leaf Node
...
uint32_t *internal_node_num_keys(void *node){
	return node + INTERNAL_NODE_NUM_KEYS_SIZE;
}
...
void *get_page(Pager *pager, uint32_t page_num){
	...
	if(pager->pages[page_num] == NULL){
		...
		//Might have to save a partial page at the end of the file
...
/*
Return the posistion of the given key.
...
void create_new_root(Table *table, uint32_t right_child_page_num){
	...
	Adderss of right child passed in.
...
void leaf_node_split_and_insert(Cursor *cursor, uint32_t key, Row *value){
	...
	*leaf_node_next_leaf(old_node) = new_page_num;
	...
	/*
	All existing keys plus new key and should be divided
	...
	for(int32_t i = LEAF_NODE_MAX_CELLS; i >= 0; i--){
		...
		if(i==cursor->cell_num){
			...
			*leaf_node_key(destination_node, index_within_node) = key;
			...
			memcpy(destination, leaf_node_cell(old_node, i-1), LEAF_NODE_LEFT_SPLIT_COUNT);
		} else {
	...
	*(leaf_node_num_cells(new_node)) = LEAF_NODE_RIGHT_SPLIT_COUNT;
	...
	/* 
	Until recyling of free pages is implemented, new pages will always 
